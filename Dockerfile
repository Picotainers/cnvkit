...
# Compatibility-first template for cnvkit.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/cnvkit" ]; then BIN="/opt/conda/bin/cnvkit"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo cnvkit | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'cnvkit*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/cnvkit
RUN chmod +x /usr/local/bin/cnvkit && rm -f /tmp/tool-entry-path
WORKDIR /data
ENTRYPOINT ["/usr/local/bin/cnvkit"]
